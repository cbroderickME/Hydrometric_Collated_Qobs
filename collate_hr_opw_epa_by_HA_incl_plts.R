...
for (basin in unique_basins) {
  ...
  # -------------------------------
  out_dir <- file.path(paste0("C:/Users/CBroderick/Downloads/EPA+OPW/Hydrometric_Collated_Qobs/Q_Output/HA_", basin, "/Hourly"))
  ...
  out_file <- file.path(out_dir, "Qobs_Hourly.txt")
  ...
  writeLines(ids_with_data, file.path(out_dir, "stations_with_data_Hourly.txt"))
  write.csv(df %>% filter(ID_pad %in% ids_with_data),
            file.path(out_dir, "stations_with_data_meta_Hourly.csv"),
            row.names = FALSE)
  ...
  if (any(time_diffs != 1)) warning("Missing days detected!")
  ...
  write.csv(summary_df,
            file.path(out_dir, "station_data_summary_Hourly.csv"),
            row.names = FALSE)
  
  # -------------------------------
  # 5g. Create comma-delimited HYPE station file
  # -------------------------------
  meta_df <- read.csv(file.path(out_dir, "stations_with_data_meta_Hourly.csv"),
  ...
  tab_out_file <- file.path(out_dir, "candidate_stations_for_hype_Hourly.txt")
  ...
  shapefile_path <- file.path(out_dir, "candidate_stations_for_hype_Hourly.shp")
  ...
  map_file <- file.path(out_dir, paste0("station_availability_map_HA_", basin, "_Hourly.html"))
